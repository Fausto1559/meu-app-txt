      ...
      <div className="min-h-screen bg-[#0c1527] flex items-center justify-center text-white p-4">
        <div className="bg-[#14223c] p-8 rounded-xl border border-slate-700 max-w-md w-full text-center space-y-6 shadow-2xl">
          ...
          <div className="space-y-4">
            {/* 1. FAZER LOGIN COM O GOOGLE (DOURADO / TEXTO BRANCO) */}
            <button 
              type="button"
              onClick={() => setUser({ email: 'usuario.google@gmail.com', displayName: 'Usuário Google' })}
              className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-3 transition-all cursor-pointer shadow-md active:scale-95 text-sm"
            >
            ...
            {/* 2. RECEBER LINK POR E-MAIL (CAMPO BORDA DOURADA + BOTÃO DOURADO / TEXTO BRANCO) */}
            <form 
              onSubmit={(e) => {
                e.preventDefault();
                const input = e.currentTarget.querySelector('input') as HTMLInputElement;
                const emailVal = input?.value || 'usuario@email.com';
                setUser({ email: emailVal, displayName: emailVal.split('@')[0] });
              }} 
              className="space-y-3"
            >
              <input
              ...
              <button 
                type="submit"
                className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 px-4 rounded-lg transition-all cursor-pointer shadow-md active:scale-95 text-sm"
              >
